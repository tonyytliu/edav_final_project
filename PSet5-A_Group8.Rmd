...
Then we will split the work by exploring at different aspects of the data. Specifically, each of us will focus on one of the three following questions.
...
The original dataset lists conflicts based on their ids. Different conflicts have different durations. Some conflicts last for one day while some others last for months. In certain parts of the future studies, we might be interested in finding the pattern based on day/month. Therefore, in the data processing stage, we may want to transform the original dataset into a new one, which records the conflict based on each date or months, as follows (the example below is based on date):

The code of the transformation process and the transformed data are on our [Project GitHub Repo](https://github.com/tonyytliu/edav_final_project).
...
In the initial investigations, we examined the number of conflicts that occurred in each year, the number of conflict-caused deaths in each year, and the geometric distribution of the conflict. Our summaries of those investigations are included in the following four subsections. Data for the following subsections are based on the conflicts' starting years in the original dataset.
